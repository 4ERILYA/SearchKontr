...
def text(update, context):
    ...
    if len(inn) == 10 and inn.isdigit():
        ...
        update.message.reply_text("ИНН указан корректно и принадлежит юридическому лицу, краткая информация об организации: "
                                  "{}".format(facts_to_str(inn_info['suggestions'][0])))
...
def get_inn_info(inn):
    if not os.path.isfile(f'{inn}.json'): # если файла нет, то считываем информацию у ФНС
        url = 'http://suggestions.dadata.ru/suggestions/api/4_1/rs/findById/party' # в бесплатной версии здесь доступно 8 видов запросов по 100 раз каждого запроса для одного API_token
        headers = { "Content-Type": "application/json",
        "Accept": "application/json", "Authorization": "Token " + API_Token }
        response = requests.post(url, headers = headers,data=json.dumps({"query": inn}))
